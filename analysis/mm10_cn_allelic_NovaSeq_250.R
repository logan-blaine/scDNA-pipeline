...
hiseq_summary=read_csv("/pellmanlab/logan/data/mouse_HiSeq/summary.csv") %>% 
  select(sample, embryo_id, guide, group, ploidy,pass_all_filters)
...
final_summary=summary_tbl %>% 
  filter(pass_all_filters & fold_coverage>=0.5)
...
segmented_data=future_map_dfr(split_df,
                              ~segmentByCBS(y=.,seed="1", avg="median",
...
future_map(unique(haplotype_cn$group), function(plot_group) {
  ...
  g=ggplot(group_cn)+
    geom_point(aes(x=bin,y=pmin(ymax,hap_cn),color=allele),size=0.3, shape=1)+
...
future_map(1:nrow(shared_cnvs), function(i) {
  ...
  ymax=pmin(max(group_cn$hap_cn, na.rm = T)+1, 6.5)
  # ymax=6.5
  chr_max=group_cn %>% group_by(contig) %>% summarize(xmax=max(bin)/1e6 + 5)
  ...
  g=ggplot(group_cn)+
    ...
    geom_point(aes(x=bin/1e6,y=pmin(ymax,hap_cn),color=allele),size=0.8, shape=1)+
    coord_cartesian(clip="off",default=T) +
    scale_x_continuous(breaks=seq(0,1e4,25), expand=c(0,0), minor_breaks=seq(0,1e4,5),name=sprintf("Position on chromosome %s (Mb)", plot_contig))+
  ...
  ggsave(file.path(plot_dir,sprintf("%s_chr%s.pdf",plot_group, plot_contig)),g,
         width=0.5+max(group_cn$bin/5e7),height=1+0.15*ymax*length(unique(group_cn$sample)))
